...
export const createBranchService = async (branch: TIBranch): Promise<string> => {
    await db.insert(BranchesTable).values(branch);
    return "Branch created successfully";
}

// Service to update a branch by ID
export const updateBranchService = async (id: number, branch: Partial<TIBranch>): Promise<string> => {
