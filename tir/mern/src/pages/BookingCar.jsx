...
function BookingCar() {
    ...
    useEffect(() => {
        if (car.rentalRate) {
            let amount = totalHours * car.rentalRate;
            if (driver) {
                amount += 30 * totalHours;
            }
            setTotalAmount(amount);
        }
    }, [driver, totalHours, car.rentalRate]);
    ...
    const placeholderImageUrl = 'https://via.placeholder.com/750x450?text=No+Image+Available';
        ...
        <DefaultLayout>
            ...
            <Row justify='center' className='d-flex align-items-center' style={{ minHeight: '90vh' }}>
                <Col lg={10} sm={24} xs={24}>
                    <img 
                        src={car.image || placeholderImageUrl} 
                        alt={car.name || 'Car'} 
                        style={{
                            height: 450,
                            width: 750,
                            ...
                            {cars.length && (
                                <div className='p-2'>
                                    {car.bookedTimeSlots.map(slot => (
                                        <Button type="primary" size='large' className='mt-3' key={slot.from + slot.to}>
                                            {slot.from} - {slot.to}
                                        </Button>
                                    ))}
                                    <div className='text-right'>
